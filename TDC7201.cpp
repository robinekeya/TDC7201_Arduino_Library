...
void TDC7201::setupOverflow(const uint8_t pinCSBx, const uint64_t overflowPs)
{
	 ...
	 *
	 * The problem is that before an actual measurement is made normLSB has not been calculated. 
	 * So for the first measurement you could use the stated nominal LSB value, 55 ps, given by the datasheet. 
	 * Doing this, COARSE_CNTR_OVF max value is FFFF (from the COARSE_CNTR_OV_H/L registers) so Timeout_value max is 227.08us.
	 * The data sheet also does not recommend using mode 1 for times greater than 2us.
	 * 
	 * Alternatively, You could perform an aborted measurement to generate a normLSB before the first actual measurement.
	 * If an accurate overflow value is necessary
	 * 
	...
	if (overflowPs)
	{
		if (m_mode == 1)
		{
			...
			const uint64_t ovf = overflowPs / (TDC7201_NOMINAL_LSB * TDC7201_RING_OSC_CELLS);
