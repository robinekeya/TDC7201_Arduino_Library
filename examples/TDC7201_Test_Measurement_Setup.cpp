...
void printRegisters(int timer=PIN_TDC7201_CSB1, int startReg=0, int stopReg=23) {
  ...
  for(int reg{startReg}; reg < stopReg+1; reg++){
    ...
    else{attachInterrupt(digitalPinToInterrupt(2),buttonPressed1,RISING); 
      Serial.print("Register "); Serial.print(reg);Serial.print(": ");
...
void timerTrigger(int timerNumber)
{
  Serial.print("Timer "); Serial.print(timerNumber, 0); Serial.print("start triggered.");
}
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(6),timerTrigger(1),RISING); 
}
